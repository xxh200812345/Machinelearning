...
sample_edited_img_path = "sample_edited.jpg"
sample_sign_img_path = "sample_sign.jpg"
...
def mask_fill_white(img, mask):
    ...
    cv2.imwrite(sample_edited_img_path, result)


# 只返回指定高度以内的区域（max，min）
def remove_small_height_regions(max_height, min_height):
    image = cv2.imread(sample_edited_img_path, cv2.IMREAD_GRAYSCALE)
    # 二值化图像
    ...
    cv2.imwrite(sample_edited_img_path, image)

    mask_fill_white(image, mask)
...
def get_main_area():
    ...
    #((y1,y2),(x1,x2))
    sign_rect=((right_point[1] + int((foot_area[0][1] - right_point[1]) * 0.6) , foot_area[0][1]),
               ((right_point[0] + int((foot_area[1][0] - right_point[0]) * 0.4)), foot_area[1][0]))
    img_part = img[
        sign_rect[0][0]:sign_rect[0][1],
        sign_rect[1][0]:sign_rect[1][1]
    ]
    ...
    mask_fill_white(img, mask)

    # 只返回指定高度以内的区域（max，min）
    remove_small_height_regions(15, 10)
